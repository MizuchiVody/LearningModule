...
while (True):
    ...
    file = open('data.txt', 'r') #In this file we will have censor data 
    a = [] #In this list we will store censor data from the file as strings 

    for line in file:
        l = len(line)
        line = line[:(l - 1)]
        a.append(line) #Add string to the list

    y = 600 #Position of the text on the y-axis

    for s in a:
        putText(img=frame, text=s, org=(900, y), fontFace=FONT_HERSHEY_DUPLEX, fontScale=1,
            color=(255, 255, 255), thickness=1, lineType=CV_8S) #Print data
        y += 50 #Update position (Go to the next line)
